...
from xenseteleop_toolkit.utils.image_utils import decompress_jpg_to_image
...
class TeleopDataAnalyzer:
    ...
    def _check_data_consistency(self):
        ...
        if inconsistent_entries:
            ...
            print(
                f"   Inconsistent entries: {inconsistent_entries[:10]}{'...' if len(inconsistent_entries) > 10 else ''}"
            )
        else:
            ...
            for arm_name, arm_data in sample_data.items():
                if isinstance(arm_data, np.ndarray):
                    ...
                    if all_values:
                        ...
                        print(
                            f"      Range (first 100 entries): min={all_values.min():.4f}, max={all_values.max():.4f}"
                        )
                else:
                    print(f"   '{arm_name}': {self.get_data_type_info(arm_data)}")
        else:
                ...
                if key == "timestamp":
                    ...
                    for cam_name, cam_data in value.items():
                        if isinstance(cam_data, np.ndarray):
                            ...
                            print(
                                f"      {cam_name}: {img_shape} (H×W×C: {img_shape[0]}×{img_shape[1]}×{img_shape[2] if len(img_shape) > 2 else 'N/A'})"
                            )
                        elif isinstance(cam_data, dict):
                            ...
                            if any(isinstance(v, np.ndarray) for v in cam_data.values()):
                                ...
                                for stream_type, stream_data in cam_data.items():
                                    if isinstance(stream_data, np.ndarray):
                                        ...
                                        print(
                                            f"        {stream_type}: {img_shape} (H×W×C: {img_shape[0]}×{img_shape[1]}×{img_shape[2] if len(img_shape) > 2 else 'N/A'})"
                                        )
                                    elif isinstance(stream_data, bytes):
                                        print(
                                            f"        {stream_type}: bytes(length={len(stream_data)}, compressed_jpg)"
                                        )
    ...
    def run_full_analysis(self):
        ...
        if "image" in self.data[0]:
            print("\n" + "=" * 80)
            user_input = (
                input("Display camera images from sample entries (first, middle, last)? (y/n): ").strip().lower()
            )
