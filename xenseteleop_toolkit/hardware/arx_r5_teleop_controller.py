...
from xenseteleop_toolkit.common.base_hardware_teleop_controller import (
    HardwareTeleopController,
)
from xenseteleop_toolkit.hardware.interface.arx_r5 import ARXR5Interface
from xenseteleop_toolkit.hardware.interface.realsense import RealSenseCameraInterface
from xenseteleop_toolkit.utils.geometry import R_HEADSET_TO_WORLD
...
class ARXR5TeleopController(HardwareTeleopController):
    ...
    def _get_camera_frame_for_logging(self) -> Dict:
        ...
        for serial, frames in frames_by_serial.items():
            camera_name = self.camera_serial_to_name.get(serial, serial)  # Fallback to serial if name not found
