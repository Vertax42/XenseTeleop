...
from xenseteleop_toolkit.common.base_teleop_controller import BaseTeleopController
from xenseteleop_toolkit.utils.geometry import (
    R_HEADSET_TO_WORLD,
)
from xenseteleop_toolkit.utils.mujoco_utils import (
...
class MujocoTeleopController(BaseTeleopController):
    ...
    def _robot_setup(self):
        ...
        for name, config in self.manipulator_config.items():
            if "vis_target" not in config:
                print(f"Warning: 'vis_target' not found in config for {name}. " f"Skipping mocap setup.")
                continue
    ...
    def _update_mocap_target(self):
        for name, task in self.effector_task.items():
            ...
            if hasattr(task, "T_world_frame"):
            ...
            elif hasattr(task, "target"):
                # PositionTask (position-only control)
                # For position-only tasks, we need to get the current position
                # from the task target and use the current orientation from
                # the robot's current state
                target_pos = task.target
                # Get current orientation from the robot's current state
                link_name = self.manipulator_config[name]["link_name"]
                current_xyz, current_quat = self._get_link_pose(link_name)

                # Create a 4x4 transformation matrix
                T_world_target = tf.quaternion_matrix(current_quat)
                T_world_target[:3, 3] = target_pos
                ...
                print(f"Warning: Unknown task type for {name}, " f"skipping mocap update")
